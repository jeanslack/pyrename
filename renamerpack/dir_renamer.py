...
# VERSION: v0.9
# DESCRIPTION: module batch renaming for folders
    ...
    if disp == 'head' or disp == 'HEAD':
        for i, filename in enumerate(filelist):
            ...
            try:
                ...
                sys.exit("\033[31;1m[ERROR]: One or more files have the "
                         "same name\033[0m")
        sys.exit() 
...
def d_rename():
    ...
    if src in li and os.path.isfile(src):
        print ("\033[31;1m[WARNING]: I can not rename the files, use the "
               "-f prefix\033[0m")
        sys.exit()
    else:
        for filename in li:
            if os.path.isdir(filename):
                ...
                try:
                    ...
                    sys.exit("\033[31;1m[ERROR]:  There is a file with the "
                             "same name\033[0m")
    sys.exit()
...
def d_delnumerate():
    ...
    for i in tmp: # itero sull'esistenza dei file
        if tmp.count(i) > 1:
            sys.exit("\033[31;1m[ERROR]: One or more folders have the "
                     "same name\033[0m")
...
def f_addname():
    """
    Aggiunge stringhe all'inizio o alla fine dei nomi 
    preservando l'estensione
    """
    ...
    if disp == 'head' or disp == 'HEAD':
        for a in FileList:
            if "%s%s" % (dest, a) in FileList:
                sys.exit("\033[31;1m[ERROR]: The object folder already "
...
def d_extreplace():
    ...
    if ".%s" % oldext not in FileExtension:
        sys.exit("\033[31;1m[ERROR]: There is no file with the extension "
                 "'.%s' in the path\033[0m" % oldext)
