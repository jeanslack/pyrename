...
# VERSION:      v0.9
# DESCRIPTION:  module batch renaming for files
...
# LAST REVISION:  28/01/2015/12/12/2017
##############################################
import os
import sys
import string
...
def f_namereplace():
    """
    rinomina e numera tutti i file trovati
    """
    FileList = []
    dirname = os.getcwd()  
    ...
    disp = raw_input("\033[0;1mChoose where to place the numbering > "
                    "[head/tail] \033[0m ")
    if disp == 'head' or disp == 'HEAD':
        for i, filename in enumerate(FileList):
            ...
            try:
                ...
                sys.exit("\033[31;1m[ERROR]: One or more folders have "
                        "the same name\033[0m")
        sys.exit() 
...
def f_rename():
    ...
    for name in li:		
        if os.path.isfile(os.path.join(name)): # filtra il contenuto
    ...
    for i in FileList: # itero sull'esistenza dei file
        if FileList.count(i) > 1:
            sys.exit("\033[31;1m[ERROR]:  There is a file with the same "
                    "name\033[0m")

    if src in li and os.path.isdir(src):
        print ("\033[31;1m[WARNING]: I can not rename the directory, use "
            "prefix -d\033[0m")
        sys.exit()		
    else:
        for filename in li:
            if os.path.isfile(filename):
                ...
                try:
                    ...
                    sys.exit("\033[31;1m[ERROR]: There is a directory "
                            "with the same name\033[0m")
    sys.exit()
    ...
    if disp == 'head' or disp == 'HEAD':
        for i, filename in enumerate(FileList):
            ...
            try:
                ...
                sys.exit("\033[31;1m[ERROR]: One or more directories have "
                        "the same name\033[0m")
        sys.exit() 
    elif disp == 'tail' or disp == 'TAIL':
        for i, filename in enumerate(FileList):
...
def f_delnumerate():
    ...
    for i in tmp:
        if tmp.count(i) > 1:
            sys.exit("\033[31;1m[ERROR]: One or more files have the same "
                    "name\033[0m")
    ...
    for i, filename in enumerate(FileList):
        ...
        try:
            ...
            sys.exit("\033[31;1m[ERROR]: One or more directories have the "
                    "same name\033[0m")
    sys.exit()
...
def f_addname():
    """
    Aggiunge stringhe all'inizio o alla fine dei nomi preservando 
    l'estensione
    """
    dest = raw_input("\033[0;1mEnter the string to be added to the "
                    "file name >\033[0m ")
    FileList = []
    dirname = os.getcwd()  
    for name in os.listdir(dirname):
    ...
    disp = raw_input("\033[0;1mChoose where to place the new string > "
                    "[head/tail] \033[0m ")
    if disp == 'head' or disp == 'HEAD':
        for a in FileList:
            if "%s%s" % (dest, a) in FileList:
                sys.exit("\033[31;1m[ERROR]: The object file already "
                        "exists with the same name \033[0m") 
        for i, filename in enumerate(FileList):
            try:
                os.rename(filename, dest+filename)
            except OSError: #'[Errno 21] Is a directory'
                sys.exit("\033[31;1m[ERROR]: There is a directory with "
                        "the same name\033[0m")
...
def f_extreplace():
    ...
    # itero sull'esistenza di file con uguale nome.newext
    for filename in FileExist: 
        if os.path.exists(filename):
            print ("\033[31;1m[ERROR]: The object already exists with the "
                "same name\033[0m")
            if os.path.isfile(filename):
    ...
    if ".%s" % oldext not in FileExtension:
        sys.exit("\033[31;1m[ERROR]: There is no file with the "
                "extension '.%s' in the path\033[0m" % oldext)
    for filename in li:
        if os.path.isfile(os.path.join(filename)):
            name, ext = os.path.splitext(filename)
            if ext.endswith(oldext):			
...
def f_addext():
    ...
    if FileList == []:
        sys.exit("\033[31;1m[ERROR]: There is no file with no extension "
                "in the path\033[0m")
    ...
    for filename in FileList:
        try:
            ...
            sys.exit("\033[31;1m[ERROR]: There is a directory with the "
                    "same name\033[0m")
    sys.exit()
