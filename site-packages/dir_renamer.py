...
def d_namereplace():
	...
	filelist.sort()
	dest = raw_input("\033[1mRinomina tutte le cartelle come: >\033[0m ")
...
def d_rename():      
	...
	if src in li and os.path.isfile(src):
		print "\033[31;1m[WARNING]: Non posso rinominare i file, usa prefisso -f\033[0m"
		sys.exit()
	else:
		for filename in li:
	...
	if disp == 'head' or disp == 'HEAD':
		for a in FileList:
			if "%s%s" % (dest, a) in FileList:
				sys.exit("\033[31;1m[ERROR]: L'oggetto cartella esiste già con uguale nome \033[0m") 
		for i, filename in enumerate(FileList):
			try:
